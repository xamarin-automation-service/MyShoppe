...
task :build_android => [:restore_packages] do
  puts "building Android project with:"
	sh "xbuild #{ANDROID_CSPROJ} /p:Configuration=Debug /t:SignAndroidPackage"
end

task :build_ios => [:restore_packages] do
  puts "building iOS project with:"
  sh "xbuild #{IOS_CSPROJ} /p:Configuration=Debug /p:Platform=iPhone /p:OutputPath='bin/iPhone/Debug/'"
end

task :build_tests => [:restore_packages] do
  puts "building UITest project with:"
	sh "xbuild #{TEST_CSPROJ} /p:Configuration=Debug"
end
